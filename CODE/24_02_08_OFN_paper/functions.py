...
def OFN(N,iCEOS,IFG,FG,TB):
   ...
   # print(TAU)
    ...
    #AF  = = (-F1 - np.sqrt(F1 ** 2 - 4 * F2 * (F0 - Log(PR)))) / (2 * F2)
    AF = (np.log(PR) - F0)/F1
    if iCEOS ==2:
...
# Función para contruir el sistema lineal en caso de existir 2 restricciones
def Ab_2x2 (R1, R2, RE1, RE2, v, idx_array, N):
    #v = [2.0488,0.9471,2.7962,2.1132,1.014,2.900,1.1427,1.5211,1.5706]
    ...
    # for i in [0,1,2,3,7,8,4,5,6]:
    #     print(v[i])
    return v

## Función para contruir el sistema lineal en caso de existir 3 restricciones
def Ab_3x3 (R1, R2, R3, RE1, RE2, RE3, v, idx_array, N):
    #v = [2.0488,0.9471,2.7962,2.1132,1.014,2.900,1.1427,1.5211,1.5706]
    ks = [0,1,2,3,4,5,6,7,8]
    A  = np.empty((3,3))      
    b  = np.empty(3)
