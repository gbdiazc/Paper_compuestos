...
#Compuestos_n = (0:"Hexane", 1:"Cyclohexane", 2:"Benzene", 3:"Heptane", 4:"Cycloheptane",
#                5:"Toluene", 6:"Octane", 7:"Cyclooctane", 8:"Ethylbenzene", 9:"Nonane",
#                10:"Indane", 11:"Decane", 12:"1-Methylnaphthalene", 13:"Undecane")
#Compuestos_n = (2:"Hexane", 3:"Cyclohexane", 4:"Benzene", 5:"Heptane", 6:"Cycloheptane",
#                7:"Toluene", 8:"Octane", 9:"Cyclooctane", 10:"Ethylbenzene", 11:"Nonane",
#                12:"Indane", 13:"Decane", 14:"1-Methylnaphthalene", 15:"Undecane")
parallel   = False
Compuestos = True
TBE_exp    = False
HCE_exp    = True
...
#components =[3,11,13,10,1,4,7,2,5,8]
#components =[2]
# components = [3]
if Compuestos:
    if TBE_exp:
        dir_save = "RESULTS/Compuestos_case_"+str(MG_case)+"_TB_exp_"
        if HCE_exp:
            dir_save += "RHC_exp/"
        else:
            dir_save += "RHC_corr/"
...
if Compuestos ==True:#593.5,#1.0170,
        ...
        for run_s in range(runs_rank):
            ...
            for run_its in runs:
                ...
                for i in range(run_its):
                    ...
                    if VR == [1,1,0,0]:
                        v = Ab_2x2 (ResVM, ResMW, RHS_VM, RHS_MW, v, idx_array, N)
                        
                    if VR == [1,0,1,0]:
                        v = Ab_2x2 (ResVM, ResCH, RHS_VM, RHS_CH, v, idx_array, N)
                        
                    if VR == [1,0,0,1]:
                        v = Ab_2x2 (ResVM, ResTB, RHS_VM, RHS_TB, v, idx_array, N)
                
                    if VR == [1,1,1,0]:
                        v, A = Ab_3x3 (ResVM, ResMW, ResCH, RHS_VM, RHS_MW, RHS_CH, v, idx_array, N)
                        
                    if VR == [1,1,0,1]:
                        v, A = Ab_3x3 (ResVM, ResMW, ResTB, RHS_VM, RHS_MW, RHS_TB, v, idx_array, N)
                        
                    if VR == [1,0,1,1]:
                        v, A = Ab_3x3 (ResVM, ResCH, ResTB, RHS_VM, RHS_CH, RHS_TB, v, idx_array, N)
                        
                    if VR == [1,1,1,1]:
                        v = Ab_4x4 ( ResVM, ResMW, ResCH, ResTB, RHS_VM, RHS_MW,  RHS_CH, RHS_TB, v, idx_array, N)
                    ...
                    if  (min(v)>=epsilon_v ):
                        ...
                        if (vlist not in feasible) and math.isinf(OFN(N,iCEOS,IFG,v,TBE)) ==False and np.isnan(OFN(N,iCEOS,IFG,v,TBE))==False and max(v)<12 :
                            ...
                            #print("OF: ",OFN(N,iCEOS,IFG,v,TBE))
                            #print("v: ",v)
                end = time.time()
