...
for component in components:
    ...
    data_1 =  {'Case':"T",'C':Compuestos_n[component],#int(component+1),
               'min OF':sols["OF"],
               '$v_1$':sols["v1"],'$v_2$':sols["v2"],
               '$v_3$':sols["v3"],'$v_4$':sols["v4"],
               '$v_5$':sols["v5"],'$v_6$':sols["v6"],
               '$v_7$':sols["v7"],'$v_8$':sols["v8"],
               '$v_9$':sols["v9"],
               '$Tb$':crit_sol["Tb"], '$Pc(%Error)$':"",'$Pc$':crit_sol["Pc"], 
               '$Tb(%Error)$':"","Tc":crit_sol["Tc"],'$Tc(%Error)$':"", 
               "Ac":crit_sol["Omega"], '$Ac(%Error)$':""}
    ...
    for case in cases:
        ...
        for run_c in np.arange(len(runs)):
            ...
            data_1 =  {'Case':case,'C':Compuestos_n[component],#int(component+1),
